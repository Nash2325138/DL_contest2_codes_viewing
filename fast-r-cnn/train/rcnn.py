...
from train.roi_pooling import *
from train.training import *


class RCNNModel(BasicModel):
    def __init__(self, name='FastRCNNModel'):
        self.name = name
    ...
    def build(self):
        ...
        self.input_layer = tf.placeholder(
            dtype=tf.float32, shape=[None, img_height, img_width, 3])
        self.input_roi = tf.placeholder(
                dtype=tf.float32, shape=[4,])
        ...
        roi_pooled = roi_pooling_layer(feature_map=norm5, roi=self.input_roi, output_shape=[40, 40])
        print('pooled', flush=True)

        flatten = tf.reshape(roi_pooled, [-1, 1600])
    ...
    def save_model(self, sess, global_step, path):
        saver = tf.train.Saver()
        fullname = os.path.join(path, 'rcnn')
