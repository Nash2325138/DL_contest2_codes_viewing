...
def main():
    ...
    with tfsession() as sess:
        ...
        if os.path.exists('./myrcnncheck'):
            model.load_model(sess, './myrcnncheck')
        step = train_model(sess, model, training_init, valid_init, next_element, df_train, df_valid, epoch=20)
